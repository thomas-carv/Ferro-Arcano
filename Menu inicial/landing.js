(() => {
  ...
  const meleeWeapons = [['Desarmado','1d4','Corpo a corpo','0'],['Faca','1d4','Adjacente','1'],['Espada curta','1d6','Adjacente','1'],['Machado pesado','1d10','Adjacente','2'],['Martelo de guerra','1d8','Adjacente','2'],['Lança','1d6','3m','1']]
  ...
  const state = {id:'',name:'',concept:'',level:1,classId:'atirador',originId:'soldado',trailIndex:0,weaponIndex:0,meleeIndex:0,shield:false,trainedSkills:[],attributes:{CON:0,CONH:0,FOC:0,FOR:0,REF:0}}
  ...
  $('#trail-choice').addEventListener('change', event => { state.trailIndex = Number(event.target.value); renderChoices() })
  ...
  $('#export-pdf').addEventListener('click', () => {
    ...
    printSheet.innerHTML = `<p>FERRO & ARCANO · v1.4b</p><h1>${esc(state.name || 'Personagem sem nome')}</h1><p class="print-muted">${esc(state.concept || 'Sem conceito')} · Nível ${state.level} · ${c[1]}</p><div class="print-grid"><div class="print-box"><b>PV máximo</b><strong>${d.pv}</strong></div><div class="print-box"><b>VA de fogo</b><strong>${d.va}</strong></div><div class="print-box"><b>Defesa</b><strong>${d.defense}</strong></div><div class="print-box"><b>${c[7]}</b><strong>${d.resource}</strong></div></div><h2>Atributos</h2><p>${Object.entries(state.attributes).map(([key,value]) => `<b>${key}</b>: ${value}`).join(' &nbsp; ')}</p><h2>Identidade</h2><p><b>Classe:</b> ${c[1]} — ${c[2]}<br><b>Origem:</b> ${origin ? origin[1] : 'Não escolhida'}<br><b>Arma:</b> ${d.weapon[0]} · ${d.weapon[1]} · ${d.weapon[2]}m · ${d.weapon[3]} munições</p>`
